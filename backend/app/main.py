...
CORS: корректные списки, без пустых строк
...
dataset: str # "waves" | "physics"
variable: str # напр. "VHM0" (waves), "thetao"/"uo"/"vo" (physics)
lat: float
lon: float
...
"""
Выполнить copernicusmarine login, если заданы учетные данные.
Без падения при отсутствии CLI.
"""
...
# CLI не установлен — не критично для старта; ошибки будут на subset.
pass
...
t_start: Optional[str], t_end: Optional[str]
) -> str:
"""
Выполнить выборку через CLI Copernicus Marine Toolbox и вернуть путь к времному NetCDF.
"""
...
try:
    ...
    raise HTTPException(status_code=500, detail="Не найден `copernicusmarine` (CLI). Установите пакет и выполните login.")
except subprocess.CalledProcessError as e:
    # Пробрасываем stderr/stdout в detail
    msg = e.stderr or e.stdout or "Ошибка copernicusmarine subset"
...
"""
Определить имена координат: (time, lat, lon). Выбрасывает исключение, если не удалось.
"""
...
"""
Конвертировать DataArray во временной ряд (ISO8601, значения, единицы)
"""
times_iso: List[str] = []
# первая размерность — это время
time_dim = da.dims[0]
...
info = {
"wmts": WMTS_BASE,
"datasets": {"waves": DATASET_WAV, "physics": DATASET_PHY},
...
# проксирование всех query-параметров к WMTS
...
# небольшой бокс вокруг точки — устойчивое nearest
eps = 0.05
...
try:
    ds = xr.open_dataset(nc_path)
    _, lat_name, lon_name = _detect_coords(ds)
    ...
    lat_step = max(1, int(req.step))
    lon_step = max(1, int(req.step))

    lat_idx = np.arange(0, len(lats), lat_step)
    lon_idx = np.arange(0, len(lons), lon_step)

    U = u.values[np.ix_(lat_idx, lon_idx)]
    V = v.values[np.ix_(lat_idx, lon_idx)]
